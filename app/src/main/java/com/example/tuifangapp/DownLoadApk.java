...
public class DownLoadApk {
	 ...
	 public static void download(Context context, String url, String title,final String appName) {
	        ...
	        if (downloadId != -1L) {
	            ...
	            if (status == DownloadManager.STATUS_SUCCESSFUL) {
	                ...
	                if (uri != null) {
	                    if (true) {
							ToolsUtil.installApk(context, ToolsUtil.getApkDir()+ "/Tuifang.apk",uri);
	                        return;
	                    } else {
	 ...
	 private static void start(Context context, String url, String title,String appName) {
	    ...
	    public static void startInstall(Context context, Uri uri) {
			...
			{
				File file= null;
					file = new File(Environment.DIRECTORY_DOWNLOADS+"Tuifang.apk");
					Uri apkUri = FileProvider.getUriForFile(context, "com.example.tuifangapp.fileprovider", file);
